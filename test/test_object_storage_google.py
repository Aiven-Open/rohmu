...
from googleapiclient.http import MediaDownloadProgress, MediaUploadProgress
from io import BytesIO
from rohmu.common.models import StorageOperation
from rohmu.object_storage.google import GoogleTransfer, MediaIoBaseDownloadHack, Reporter
...
def test_media_io_download_with_byte_range() -> None:
    ...
    result = BytesIO()
    download = MediaIoBaseDownloadHack(result, mock_request, byte_range=(3, 8))
    status, done = download.next_chunk()
    assert done
    assert status.progress() == 1.0
    assert result.getvalue() == b"lo, Wo"
    assert mock_request.http.request.mock_calls[0].kwargs["headers"]["range"] == "bytes=3-8"
...
def test_media_io_download_with_byte_range_and_tiny_chunks() -> None:
    ...
    result = BytesIO()
    download = MediaIoBaseDownloadHack(result, mock_request, chunksize=3, byte_range=(3, 10))
    status, done = download.next_chunk()
    assert not done
    assert status.progress() == 0.375
    assert result.getvalue() == b"lo,"
    assert mock_request.http.request.mock_calls[0].kwargs["headers"]["range"] == "bytes=3-5"

    status, done = download.next_chunk()
    assert not done
    assert status.progress() == 0.750
    assert result.getvalue() == b"lo, Wo"
    assert mock_request.http.request.mock_calls[1].kwargs["headers"]["range"] == "bytes=6-8"

    status, done = download.next_chunk()
    assert done
    assert status.progress() == 1.0
    assert result.getvalue() == b"lo, Worl"
    assert mock_request.http.request.mock_calls[2].kwargs["headers"]["range"] == "bytes=9-10"
